...
type Props = {
  ...
  editImageFile: File | null; // ✅ requerido (y ya lo pasas desde el page)
      ...
      <div className="bg-white rounded-[32px] shadow-2xl w-full max-w-5xl px-8 py-6 relative">
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="w-full max-w-sm mx-auto lg:mx-0">
            ...
            {editImageFile && (
              <p className="mt-2 text-[11px] text-slate-500">
                Archivo: <span className="font-semibold">{editImageFile.name}</span>
              </p>
            )}
          </div>
          ...
          <div className="flex-1 grid gap-6 lg:grid-cols-[2.3fr,1.2fr]">
            ...
            {/* Historial */}
            <aside className="bg-slate-50 rounded-2xl border border-slate-200 p-3 text-[11px] flex flex-col max-h-64 lg:max-h-[320px]">
              <h3 className="text-[11px] font-semibold text-slate-700 mb-2">
                Historial de cobertura
              </h3>

              {loadingHistorial ? (
                <p className="text-[11px] text-slate-500">Cargando historial…</p>
              ) : historial.length === 0 ? (
                <p className="text-[11px] text-slate-500">Sin movimientos registrados.</p>
              ) : (
                  ...
                  {historial.map((h) => (
                    ...
                    >
                      <span className="font-semibold text-slate-800">
                        {h.fecha_ingreso} {h.fecha_retiro ? `→ ${h.fecha_retiro}` : "→ Actualidad"}
                      </span>
                      <span className={h.estado === "en_cobertura" ? "text-emerald-600" : "text-rose-600"}>
                        {h.estado === "en_cobertura" ? "En cobertura" : "Retirado"}
                      </span>
                      {h.created_at && (
